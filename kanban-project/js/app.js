...
(function() {
    ...
    function validateForm() {
        ...
        if (dueDateInput.value) {
            const selectedDate = new Date(dueDateInput.value).setHours(0,0,0,0);
            const today = new Date().setHours(0,0,0,0);
...
function deleteTask(taskId) {
    ...
    function toggleSubtaskPanel(cardElement) {
        ...
        if (!panel.classList.contains('show')) {
            // If we are closing it, remove rendered subtasks to save memory
            panel.querySelector('.subtask-list').innerHTML = '';
        } else {
            // If we are opening it, render the subtasks
